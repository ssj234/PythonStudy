...
def add_layer(inputs,in_size,out_size,activation_function=None):
    Weights = tf.Variable(tf.random_normal([in_size,out_size]))  # matrix so Upper
    biases = tf.Variable(tf.zeros([1,out_size]) + 0.1)  # a list so Lower not zero so add 0.1
...
#
xs = tf.placeholder(tf.float32,[None,1]) #None:多少行都可以
